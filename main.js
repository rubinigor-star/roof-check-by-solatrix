...
function calculateReport() {
  ...
  const daytimeUseLimit = annualProduction * 0.45;
  const selfConsumed = Math.min(daytimeUseLimit, annualConsumption);
...
function markRoof() {
  if (state.roofType === 'sloped') addRoofSide();
  else {
    const nextIndex = state.surfaces.length ? state.surfaces[0].id % shapePresets.length : 0;
    state.surfaces = [createSurface(nextIndex)];
    render();
  }
}
...
function header() {
  return `<header class="siteHeader ${state.menuOpen ? 'menuOpen' : ''}">
    ...
    <nav class="mobileMenu" aria-hidden="${state.menuOpen ? 'false' : 'true'}">
      ...
      <button data-action="step:1">כתובת וחשבון</button>
...
function mapMock(interactive = false) {
  ...
  const action = interactive ? 'data-action="markRoof"' : '';
  const hint = interactive ? '<div class="markHint">לחצו על המפה כדי לסמן את שטח הגג</div>' : '';
  return `<div class="mapPanel ${interactive ? 'interactiveMap' : ''}" ${action}><div class="mapBadge">${state.surfaces.length ? 'Roof marked' : 'Tap to mark'}</div>${hint}<div class="scanPulse"></div><svg class="roofCanvas" viewBox="0 0 100 100"><defs><pattern id="grid" width="8" height="8" patternUnits="userSpaceOnUse"><path d="M 8 0 L 0 0 0 8" fill="none" /></pattern></defs><rect x="0" y="0" width="100" height="100" class="mapBase"></rect><rect x="0" y="0" width="100" height="100" fill="url(#grid)" class="mapGrid"></rect><path class="sunRay" d="M5 15 L35 42 M4 38 L34 52 M12 60 L42 62"></path><path class="building" d="M12 14 L86 9 L92 82 L18 90 Z"></path>${surfaceShapes}<g class="obstaclePins">${pins}</g></svg></div>`;
}
...
function addressScreen() {
  return `<section class="screen">${floatingDecor()}<div class="card focusCard">${cardDecor()}${miniSolarGraphic()}${progress()}<div class="screenIcon">📍</div><h2>כתובת וחשבון חשמל</h2><p class="subText">החשבון החודשי עוזר לחשב כמה מהייצור יחסוך קנייה ב־₪${CONFIG.buyRate} וכמה יימכר ב־₪${CONFIG.sellRate}.</p><div class="fieldGroup"><label>כתובת הגג</label><input value="${state.address}" placeholder="לדוגמה: החרמון 10, חיפה" data-field="address" /></div><div class="fieldGroup"><label>חשבון חשמל חודשי משוער</label><input value="${state.monthlyBill}" inputmode="numeric" placeholder="לדוגמה: 850" data-field="monthlyBill" /></div>${actions('מצא את הגג')}</div></section>`;
}
...
function drawScreen() {
  ...
  return `<section class="screen mapScreen"><div class="card mapCard">${cardDecor()}${progress()}<div class="screenIcon">✏️</div><h2>${isSloped ? 'סמנו כל צד של הגג' : 'סמנו את שטח הגג'}</h2>${mapMock(true)}<div class="markStatus">${count ? `סומנו ${count} שטחי גג` : 'עדיין לא סומן שטח. לחצו על המפה או על הכפתור.'}</div><div class="drawFooter">${isSloped ? `<div class="sideCount">${count} צדדים סומנו</div>` : ''}<div class="actions compactActions">${isSloped ? '<button class="primaryBtn" data-action="markRoof">+ סמן צד נוסף</button>' : '<button class="primaryBtn" data-action="markRoof">סמן גג</button>'}${isSloped && count > 0 ? '<button class="ghostBtn" data-action="removeSide">בטל אחרון</button>' : ''}</div><button class="nextTextBtn" data-action="next" ${count === 0 ? 'disabled' : ''}>סיימתי</button></div></div></section>`;
}
...
function analysisScreen() {
  return `<section class="screen">${floatingDecor()}<div class="card centerCard analysisCard">${cardDecor()}<div class="loader"></div><h2>מנתחים את הגג...</h2><p class="subText">בודקים שטח שימושי, כיוונים, צריכה עצמית, מכירה לרשת והחזר השקעה.</p><div class="analysisBadges"><span>שטח</span><span>ייצור</span><span>תעריף משולב</span><span>ROI</span></div></div></section>`;
}

function tariffMix(report) {
  const selfWidth = Math.max(8, Math.min(92, report.selfUseShare));
  const exportWidth = Math.max(8, 100 - selfWidth);
  return `<div class="tariffMix"><div class="mixHead"><b>תמהיל חיסכון ומכירה</b><span>תעריף אפקטיבי: ₪${report.effectiveTariff.toFixed(2)} לקוט״ש</span></div><div class="mixBar"><i style="width:${selfWidth}%"></i><em style="width:${exportWidth}%"></em></div><div class="mixLegend"><span><i></i>צריכה עצמית: ${formatNumber(report.selfConsumed)} kWh לפי ₪${CONFIG.buyRate}</span><span><em></em>מכירה לרשת: ${formatNumber(report.exported)} kWh לפי ₪${CONFIG.sellRate}</span></div></div>`;
}
...
function reportRows(report) {
  return [
    ['גודל מערכת משוער', `${report.systemKw.toFixed(1)} kW`],
    ['שטח גג', `${formatNumber(report.roofArea)} m²`],
    ['שטח שימושי', `${formatNumber(report.usableArea)} m²`],
    ['מספר פאנלים משוער', `${report.panels}`],
    ['ייצור שנתי', `${formatNumber(report.annualProduction)} kWh`],
    ['צריכה עצמית', `${formatNumber(report.selfConsumed)} kWh`],
    ['מכירה לרשת', `${formatNumber(report.exported)} kWh`],
    ['חיסכון/הכנסה שנתית', `${formatMoney(report.annualSavings)}`],
    ['תעריף אפקטיבי', `₪${report.effectiveTariff.toFixed(2)} / kWh`],
    ['החזר השקעה', `${report.payback.toFixed(1)} שנים`],
    ['רווח צפוי ל-25 שנה', `${formatMoney(report.profit25)}`]
  ];
}
...
function generatePdfReport() {
  ...
  const selfWidth = Math.max(8, Math.min(92, report.selfUseShare));
  const exportWidth = Math.max(8, 100 - selfWidth);
  const html = `<!doctype html><html dir="rtl"><head><meta charset="utf-8"><title>דוח Solatrix</title><style>
    @page{size:A4;margin:14mm}*{box-sizing:border-box}body{margin:0;font-family:Assistant,Arial,sans-serif;background:#f7efe1;color:#101820}.pdfPage{background:white;min-height:100vh;border-radius:24px;overflow:hidden;border:1px solid #eadbc7}.pdfHero{background:linear-gradient(135deg,#071b2f,#123a5c);color:white;padding:28px;position:relative}.pdfHero:after{content:'☀️';position:absolute;left:26px;top:22px;font-size:48px}.pdfLogo{width:150px;background:white;border-radius:18px;padding:6px 12px;margin-bottom:24px}.pdfHero h1{font-size:38px;line-height:1;margin:0 0 10px}.pdfHero p{margin:0;font-size:17px;color:#ffe1a7}.pdfContent{padding:26px}.pdfGrid{display:grid;grid-template-columns:repeat(2,1fr);gap:12px;margin:20px 0}.pdfMetric{border:1px solid #eadbc7;background:#fff9ef;border-radius:18px;padding:15px}.pdfMetric span{display:block;color:#7b6b58;font-weight:800;font-size:12px}.pdfMetric b{display:block;font-size:22px;margin-top:6px;direction:ltr;text-align:right}.pdfSection{margin-top:22px;border:1px solid #eadbc7;border-radius:22px;padding:18px;background:#fffdf9}.pdfSection h2{margin:0 0 12px;font-size:22px}.pdfMix{height:22px;border-radius:999px;overflow:hidden;display:flex;background:#eee}.pdfMix i{display:block;background:linear-gradient(90deg,#f5a11a,#ffbf54)}.pdfMix em{display:block;background:#071b2f}.pdfLegend{display:flex;gap:14px;flex-wrap:wrap;margin-top:10px;font-weight:900;color:#6d6258}.pdfBars{display:grid;gap:10px}.pdfBar{display:grid;grid-template-columns:150px 1fr;gap:12px;align-items:center}.pdfBar span{font-weight:900}.pdfBar i{display:block;height:15px;border-radius:999px;background:linear-gradient(90deg,#f5a11a,#ffbf54)}.pdfSteps{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-top:14px}.pdfStep{background:#071b2f;color:white;border-radius:18px;padding:14px;text-align:center;font-weight:900}.pdfFooter{padding:20px 26px;background:#fff7e8;color:#6d6258;font-weight:800}.noPrint{position:fixed;left:18px;bottom:18px;border:0;border-radius:999px;background:#25D366;color:white;padding:14px 20px;font-weight:900}@media print{.noPrint{display:none}.pdfPage{border:0;border-radius:0}body{background:white}}
  </style></head><body><button class="noPrint" onclick="window.print()">שמירה / הדפסה ל-PDF</button><main class="pdfPage"><section class="pdfHero"><img class="pdfLogo" src="${LOGO_SRC}" /><h1>דוח התאמה סולארי ראשוני</h1><p>${customer} · ${address}</p></section><section class="pdfContent"><div class="pdfGrid">${rows}</div><section class="pdfSection"><h2>תמהיל כלכלי</h2><div class="pdfMix"><i style="width:${selfWidth}%"></i><em style="width:${exportWidth}%"></em></div><div class="pdfLegend"><span>צריכה עצמית לפי ₪${CONFIG.buyRate}: ${formatNumber(report.selfConsumed)} kWh</span><span>מכירה לרשת לפי ₪${CONFIG.sellRate}: ${formatNumber(report.exported)} kWh</span></div></section><section class="pdfSection"><h2>גרף החזר והשפעה</h2><div class="pdfBars"><div class="pdfBar"><span>ייצור שנתי</span><i style="width:94%"></i></div><div class="pdfBar"><span>חיסכון שנתי</span><i style="width:76%"></i></div><div class="pdfBar"><span>רווח ל-25 שנה</span><i style="width:88%"></i></div></div></section><section class="pdfSection"><h2>מה נכלל בבדיקה</h2><div class="pdfSteps"><div class="pdfStep">שטח גג</div><div class="pdfStep">פאנלים</div><div class="pdfStep">ייצור</div><div class="pdfStep">החזר</div></div><p>מכשולים: ${obstacles}. הנחות בסיס: ${CONFIG.productionPerKw} kWh/kWp/year, קנייה ₪${CONFIG.buyRate}/kWh, מכירה ₪${CONFIG.sellRate}/kWh, עלות התקנה משוערת ₪${CONFIG.installCostPerKw}/kW.</p></section><section class="pdfSection"><h2>המלצת Solatrix</h2><p>הגג נראה מתאים לבדיקה מקצועית מלאה. השלב הבא הוא אימות מדידות, בדיקת תשתית חשמל, תכנון פריסת פאנלים והצעת מחיר סופית.</p></section></section><footer class="pdfFooter">Solatrix Energy · WhatsApp ${CONFIG.defaultPhone} · דוח אוטומטי ראשוני</footer></main><script>setTimeout(()=>window.print(),500)</script></body></html>`;
