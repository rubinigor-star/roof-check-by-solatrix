...
const state = { step: 0, address: '', monthlyBill: 850, roofType: '', leadName: '', leadPhone: '', surfaces: [], obstacles: [], leadSent: false, menuOpen: false, analysisTimer: null };
...
function generatePdfReport() {
  const html = buildFullPdfReport({ report: calculateReport(), state, config: CONFIG, logoSrc: LOGO_SRC, formatNumber, formatMoney });
  const win = window.open('', '_blank');
  if (!win) return;
  win.document.open(); win.document.write(html); win.document.close(); state.leadSent = true; render();
}
function renderScreen() { if (state.step === 0) return heroScreen(); if (state.step === 1) return addressScreen(); if (state.step === 2) return roofScreen(); if (state.step === 3) return drawScreen(); if (state.step === 4) return obstaclesScreen(); if (state.step === 5) return analysisScreen(); return reportScreen(); }
function render() {
